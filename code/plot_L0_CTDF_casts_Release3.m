...
% $$$     figure, plot(temp,dye_raw,'.')
    ...
    [cm,conc] = make_DYE_colormap([0 6],1);
    %    
    figure,
    scatter(temp_sort,pres_sort,15,log2(dye_sort),'filled'), set(gca,'ydir','reverse','ticklabelinterpreter','latex'), colormap(cm), caxis([0 6])
    ...
    set(ax1,'ydir','reverse','ticklabelinterpreter','latex'), colormap(ax1,cm), caxis(ax1,[0 6]),
    ylabel('Depth [m]','interpreter','latex')
    cb1=colorbar;
    ticks = get(cb1,'ytick');
    ...
    colormap(ax2,cmocean('thermal')), caxis(ax2,[16 20])
    ...
    %
    angle = -70;
    [x,y] = lltoxy_AQR(latitude,longitude,angle);
    y_grid = ones(Np,1)*y;
    x_grid = ones(Np,1)*x;
    %
    % break into two periods (early, late) to showcase shoaling
    t0 = datenum('11-Jul-2024 18:30:00');
    early = find(time< t0);
    late  = find(time> t0);    
    ...
    [lat_early, lon_early] = xytoll_AQR(xi,ybar_early*ones(1,length(xi)),angle);
    xbar_late     = nanmean(x(late));
    [lat_late, lon_late] = xytoll_AQR(xbar_late*ones(1,length(yi)),yi,angle);
    %
    figure,
...
% $$$     scatter(x_grid(early_grid),pres_grid(early_grid),20,temp_grid(early_grid),'filled'),
    ...
    Dmax_z = squeeze(nanmax( Di, [] , 3));
    mask_z = (Dsum_z>=1 & Dmax_z>=1);
    norm   = squeeze(sum(~isnan(Di),3));
    Dbar_z = Dsum_z./norm;
    ...
    set(ax1,'ydir','normal','ticklabelinterpreter','latex'), colormap(ax1,cm), caxis(ax1,[0 6]),
