function [xr, yr] = lltoxy_AQR(Lat, Lon, Angle)
%
% USAGE: [xr, yr] = lltoxy_ARQ(Lat, Lon, Angle)
%
% Converts an array of lat,lons to local X,Y (meters) relative to an origin
% and rotated to have the x-axis aligned with the ~10m isobath at Arroyo Quemado
% original: B.Woodward  14 Nov,2008
% modifed: D. Grimes  1 Sept,2023

%%%%%%%%  CONSTANTS  %%%%%%%%%
deg2rad = pi/180;
rad2deg = 180/pi;
OriginLat = 34.4693000;   % Dye releases at M2
OriginLon = -120.1277833;
if exist('Angle','var')
    theta = Angle;
else
    theta = -45;%-72.5; % North/South orientation
end
ZoneNumber = 10;    %UTM zone for Arroyo Quemado
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
