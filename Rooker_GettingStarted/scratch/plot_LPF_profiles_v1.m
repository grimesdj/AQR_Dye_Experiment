...
Temp_cont = M1.Moor.Temperature([1 2 3 4 5 7 8 9 10], M1.Moor.valid);
...
Ttime = Ttime(idx);
% MAB Vectors
top = M1.ADCP.maxRNG(M1.LPF.valid) + M1.Moor.Temperature_mab(1);
%top = top(idx);
o = ones(size(top));
keyboard
mabs = Temp_mab;
mab_mtx = top;
mab_mtx = [mab_mtx; mabs * o];
o2 = ones(size(Temp_mab));
o2 = [1;o2];
Tim_mtx = o2 * Ttime;


contour(Tim_mtx,mab_mtx,  Temp, [16 17 17.5 18], 'k', 'LineWidth', 1.5)
...
% coherence
[Cxy, f] = mscohere(T_bin, N_bin, window, noverlap, [], 1/dt);
