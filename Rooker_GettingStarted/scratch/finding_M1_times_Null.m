...
% I dont know if this returns the right thing 
...
for i = 1:length(files)
    ...
    if ~contains(files(i).name, 'config')  &&  ~contains(files(i).name, 'min')
        ...
        for  releaseNum = 1:3
            ...
            if any(ADCP.(ind).Time >= datenum(TRange.StartTime_UTC_(releaseNum)) & ADCP.M1_1.Time <= datenum(TRange.EndTime_UTC_(releaseNum)))
                M1.(Num) = load([files(i).folder,filesep,files(i).name]);
                disp([Num ' corresponds to ' ind])
            end
